...
int start_server(char *ifaces, int port, int is_threaded){
    ...
    if(is_threaded == 0){
        printf("What? This shouldn't activate. This print statement is only here because this c file won't compile otherwise!");
    }
...
int process_cli_requests(int svr_socket){
    ...
    while(1){
        ...
        if (rc == OK_EXIT || rc == STOP_SERVER_SC) {
            break;
        }
    }
...
int exec_client_requests(int cli_socket) {
    ...
    while(1) {
        ...
        Built_In_Cmds bi = rsh_match_command(cmd_list.commands[0].argv[0]);

        if (bi == BI_CMD_EXIT) {
            send_message_string(cli_socket, RCMD_MSG_CLIENT_EXITED);
            // free any allocated buffers in the command list before exiting the loop
        ...
        if (bi != BI_NOT_BI) {
            // Redirect stdout and stderr to the client socket for built-in commands.
...
static bool has_explicit_input_redirection(cmd_buff_t *cmd) {
...
static bool has_explicit_output_redirection(cmd_buff_t *cmd) {
    for (int i = 0; i < cmd->argc; i++) {
...
int rsh_execute_pipeline(int cli_sock, command_list_t *clist) {
    ...
    for (int i = 0; i < clist->num; i++) {
        ...
        if (pids[i] == 0) { // Child process
            if (i == 0) {
                if (!has_explicit_input_redirection(&clist->commands[i])) {
            ...
            if (i == num_cmds - 1) {
                if (!has_explicit_output_redirection(&clist->commands[i])) {
