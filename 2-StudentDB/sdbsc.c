...
int add_student(int fd, int id, char *fname, char *lname, int gpa) {
    //Calc offset
    int offset = (int)id * STUDENT_RECORD_SIZE;

    if (lseek(fd, offset, SEEK_SET) == (int)-1) {
        printf(M_ERR_DB_READ);
        return ERR_DB_FILE;
    }
    ...
    ssize_t bytesRead = read(fd, &currentStudent, STUDENT_RECORD_SIZE);
    ...
    if (bytesRead == -1) {
        ...
        // No data
    } else if (memcmp(&currentStudent, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE) != 0) {
...
int del_student(int fd, int id) {
    ...
    if (bytesRead == -1) {
        printf(M_ERR_DB_READ);
        return ERR_DB_FILE;
    } else if (bytesRead == 0 || memcmp(&currentStudent, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE) == 0) {
        printf(M_STD_NOT_FND_MSG, id);
        return ERR_DB_OP;
    }

    //Write empty entry to delete student
    if (lseek(fd, offset, SEEK_SET) == (int)-1) {
        printf(M_ERR_DB_READ);
        return ERR_DB_FILE;
    }
